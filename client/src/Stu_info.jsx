...
function Stu_info() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const { studentId, stuName, birthdate, gender } = formData;
    ...
    try {
      if (isEditing) {
        const res = await axios.put(`http://localhost:5000/studentinfo/${editId}`, {
          stuName,
          birthdate,
          gender,
        });
        ...
        const res = await axios.post("http://localhost:5000/studentinfo", formData);
        toast.success("Student saved successfully");
      }

      setFormData({ studentId: "", stuName: "", birthdate: "", gender: "" });
      getAllStudents();
      navigate("/bookdetail");
              ...
              {studentList.length > 0 ? (
                studentList.map((item, index) => (
                  <tr key={item._id}>
                    ...
                    <td>
                      <button className="btn btn-danger btn-sm" onClick={() => Deleterow(item._id)}>
                        Delete
                      </button>
                    </td>
                  </tr>
